...
public class AddingAdditionalServicesAction implements IAction {
    ...
    public  AddingAdditionalServicesAction() throws IllegalAccessException {
    ...
    public void execute() {
        ...
        if (!serviceNumbers.isEmpty()){
            for (int i = 0; i < serviceNumbers.size(); i++){
                ...
                if (serviceNumber == 1) {
                    serviceTypes.add(ServiceType.BREAKFAST);
                } else if (serviceNumber == 2) {
                    serviceTypes.add(ServiceType.LYNCH);
                } else if (serviceNumber == 3) {
                    serviceTypes.add(ServiceType.DINNER);
                } else if (serviceNumber == 4) {
                    serviceTypes.add(ServiceType.CLEANING);
                } else if (serviceNumber == 5) {
                    serviceTypes.add(ServiceType.SPA);
                } else {
                    consoleView.display("Invalid service numberвавава: " + serviceNumber);
                }
            }
        }
